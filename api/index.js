...
<head>
    ...
    <style>
        ...
        button { background:#34d399; color:#0f2a22; border:none; padding:16px 40px; font-size:1.1rem; font-weight:700; border-radius:12px; cursor:pointer; margin-top:30px; transition:all 0.3s; }
...
export default async function handler(req, res) {
  // ====================== POST /shorten ======================
  if (req.method === 'POST') {
    // IMPORTANT: We read raw body because Vercel bodyParser can be unreliable in some setups
    let body = '';
  ...
  // ====================== GET /{shortcode} ======================
  if (req.method === 'GET') {
    // FIXED: Ignore any query parameters (e.g. ?shortcode=xxx)
...
// VERY IMPORTANT for Vercel when reading raw body manually
export const config = {
  api: {
    bodyParser: false,
  },
